 ...
 * /src/main/java/org/apache/hadoop/yarn/applications/distributedshell/
 * 
 * Modified by Ioannis Paraskevakos to facilitate RADICAL Pilot Launch */

package com.radical.pilot;
...
public class Client {
  ...
  private String appName = "RadicalPilot";
  // App master priority
  ...
  private String appMasterMainClass="com.radical.pilot.ApplicationMaster";
  ...
  public static final String UNIX_SCRIPT_PATH = "ExecScript.sh";
  
  private String serviceUrl = "yarn://localhost?fs=hdfs://localhost:9000";
  ...
  public Client(Configuration conf) throws Exception  {
    this(
      "com.radical.pilot.ApplicationMaster",
      conf);
  }
  ...
  Client(String appMasterMainClass, Configuration conf) {
    ...
    opts = new Options();
    opts.addOption("appname", true, "Application Name. Default value - RadicalPIlot");
    ...
    opts.addOption("service_url", true, "URL of YARN/HDFS service, e.g. yarn://localhost?fs=hdfs://localhost:9000");
    opts.addOption("debug", false, "Dump out debug information");
    opts.addOption("help", false, "Print usage");
    /*Need to add option for YARN/HDFS URL*/
  ...
  public boolean init(String[] args) throws ParseException {
    ...
    LOG.info("Debug Information is enabled."+debugFlag);

    appName = cliParser.getOptionValue("appname", "RadicalPilot");
    ...
    /*Need to add the url for YARN and HDFS before initializing the yarn client*/
    ...
    LOG.info("YARN RM address: "+conf.get("yarn.resourcemanager.address"));
    yarnClient.init(conf);
   try {
        Thread.sleep(100);
      } catch (InterruptedException e) {
        LOG.info("Thread sleep in monitoring loop interrupted");
      }
  ...
  public boolean run() throws IOException, YarnException {
    ...
    if (!shellScriptPath.isEmpty()) {
      ...
      String shellPathSuffix =
          appName + "/" + appId.toString() + "/" + UNIX_SCRIPT_PATH;
      Path shellDst =
          new Path(fs.getHomeDirectory(), shellPathSuffix);
      fs.copyFromLocalFile(false, true, shellSrc, shellDst);
      hdfsShellScriptLocation = shellDst.toUri().toString(); 
      LOG.info("SCRIPT DESINATION: "+ hdfsShellScriptLocation);
    ...
    if (debugFlag) {
      LOG.info("Enabling Debug Info in Aplication Manager");
      vargs.add("--debug");
    }
...
private boolean monitorApplication(ApplicationId appId)
  ...
  {
    PrintWriter writer = new PrintWriter("YarnApplicationReport.log", "UTF-8");
    writer.println("#-------------------------------------------------------------------------------");
    
    writer.println("YARN Application report");
    writer.flush();
    for (;;)
    {
      try
      ...
      if ((state == YarnApplicationState.ACCEPTED) && (!this.acceptedState))
      {
        writer.println("appId=" + appId.getId() + ",appStartTime=" + report
          .getStartTime() + ",yarnAppState=" + state
          .toString() + ",StateTime=" + 
          System.currentTimeMillis() + ",distributedFinalState=" + report
          .getFinalApplicationStatus().toString());
        writer.flush();
        this.acceptedState = true;
      }
      if ((state == YarnApplicationState.RUNNING) && (!this.runningState))
      {
        writer.println("appId=" + appId.getId() + ",appStartTime=" + report
          .getStartTime() + ",yarnAppState=" + state
          .toString() + ",StateTime=" + 
      ...
      if (YarnApplicationState.FINISHED == state)
      {
        if (FinalApplicationStatus.SUCCEEDED == dsStatus)
        {
          ...
          writer.println("Application has completed successfully. Breaking monitoring loop");
          writer.close();
          return true;
        }
        LOG.info("Application did finished unsuccessfully. YarnState=" + state
          .toString() + ", DSFinalStatus=" + dsStatus.toString() + ". Breaking monitoring loop");
        
        writer.println("Application did finished unsuccessfully. YarnState=" + state
          .toString() + ", DSFinalStatus=" + dsStatus.toString() + ". Breaking monitoring loop");
      ...
      {
        LOG.info("Application did not finish. YarnState=" + state
          .toString() + ", DSFinalStatus=" + dsStatus.toString() + ". Breaking monitoring loop");
        
        writer.println("Application did not finish. YarnState=" + state
          .toString() + ", DSFinalStatus=" + dsStatus.toString() + ". Breaking monitoring loop");
      ...
      if (System.currentTimeMillis() > this.clientStartTime + this.clientTimeout)
      {
        ...
        writer.println("Reached client specified timeout for application. Killing application");
  ...
  private void addToLocalResources(FileSystem fs, String fileSrcPath,
    ...
    LOG.info("File Destination: "+suffix);
    Path dst =
