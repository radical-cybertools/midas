...
if __name__ == '__main__':
	...
	atoms = list()
        ...
        #---- Calculate euclidean distances of all atoms and save them to a file----------------------
        output = open("distances_%d_%d.data" % (reading_start_point_i,j_dim), "w") 
            ...
            for i in range(0,WINDOW_SIZE):
            	...
            	for j in range(i+1,WINDOW_SIZE):
            		...
            		if dist<=cutoff:
            			...
            			distances.append(0)
            	dist_to_string = ','.join(map(str,distances))
            	output.write(dist_to_string)
            	output.write("\n")
        else:
            for i in range(0,WINDOW_SIZE):
                distances = list()
