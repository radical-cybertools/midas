...
if __name__ == "__main__":
    ...
    partitions = 100
    if len(sys.argv)!=4:
        print 'Usage: python submit_bot_Stampede.py <cores>  <partitions> <report_name>'
        sys.exit(-1)
    else:
    ...
    try:
        ...
        for i in range(NUMBER_JOBS):
            ...
            cudesc = rp.ComputeUnitDescription()
            cudesc.executable = "spark-submit"
            cudesc.arguments = ['--conf spark.cores.max=1 --conf spark.dynamicAllocation.schedulerBacklogTimeout=5s --conf spark.dynamicAllocation.enabled=true --conf spark.shuffle.service.enabled=true --conf spark.driver.maxResultSize=5g --executor-memory 5g --driver-memory 30g sleep.py %d' % partitions]
        ...
        ProfFile = open('leafletNoSparkProfile-{0}.csv'.format(cores),'w')
        ProfFile.write('CU,Name,StageIn,Allocate,Exec,StageOut,Done\n')
        for cu in cu_set:
            timing_str=[cu.uid,'N/A','N/A','N/A','N/A','N/A']
